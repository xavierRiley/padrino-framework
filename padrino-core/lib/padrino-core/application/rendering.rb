module Padrino
  ...
  # template engine detection, enhanced layout functionality, locale enabled 
  # rendering, among other features.
  #
  class Application
    ##
    ...
    def resolve_template(template_path, options={})
      ...
      options.reverse_merge!(Rendering::DEFAULT_OPTIONS)
      ...
      templates = Dir[File.join(view_path, template_path) + ".*"].map do |file|
        ...
        [template_file, template_engine] unless Padrino::EXCLUDE_PATTERNS.any? { |pattern| template_engine.to_s =~ pattern }
      end
