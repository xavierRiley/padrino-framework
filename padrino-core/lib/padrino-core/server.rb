module Padrino
  ...
  class Server < Rack::Server
    ...
    def start
      ...
      $-w = true if options[:warn]
      # Append given patsh to LOAD_PATH...
      includes = options[:include] and $:.unshift(*includes)
      # Require given library...
      library = options[:require] and require library 
      # Run in backgraund if enabled...
      daemonize_app if options[:daemonize]
      # Write pid to given file if enabled...
      write_pid if options[:pid]
      # Run padrino application within specified server...
